...
namespace Peng
{
	...
	template <class T>    //define key order.
	class bucket
	{
	public:
		bucket()
	...
	public:
		...
		void increase()//forward order
		{
			for (int i = -9; i < 10; i++)
				bucket_array[i] = i + 9;
		}

		void decrease()//reverse order
		{
			for (int i = -9; i < 10; i++)
				bucket_array[i] = 9 - i;
		}
	...
	void integer_lsd_bucket_sort(vector<int>& input, bool direct = true);
